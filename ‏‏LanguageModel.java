...
public class LanguageModel {
    ...
    /**
     * Constructs a language model with the given window length. Generating texts
     * from this model multiple times will produce different random texts. Good for
     * production.
     */
    ...
    public void calculateProbabilities(List probs) {
        ...
        for (int j = 1; j < probs.getSize(); j++) {
            current = probs.get(j);
            double p = current.count / (double) charsNum;
            current.p = p;
     ...
     *                         Map, we generate no text and return only the initial
     *                         text.
